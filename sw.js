...
self.addEventListener('fetch',e=>{const req=e.request;if(req.method!=='GET')return;
  ...
  if(same||/gstatic\.com\/firebasejs|fonts\.googleapis|fonts\.gstatic/.test(req.url)){
    e.respondWith(fetch(req,{cache:'no-cache'}).then(res=>{if(res&&(res.ok||res.type==='opaque')){const copy=res.clone();caches.open(VERSION).then(c=>c.put(req,copy))}return res})
